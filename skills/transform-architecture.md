...
> **IMPLEMENTED** in Phase 4. See `server/lib/vba-intent-extractor.js`, `server/lib/vba-intent-mapper.js`, and `skills/intent-extraction.md` for the working implementation. The module viewer's "Extract Intents" button drives intent extraction for LLM reasoning. Runtime event execution uses JavaScript handlers generated by `server/lib/vba-to-js.js`.
