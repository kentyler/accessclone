...
module.exports = function(router, pool) {
  ...
  /**
   * GET /api/database-import/browse
  ...
  router.get('/browse', async (req, res) => {
    try {
      ...
      logError(pool, 'GET /api/database-import/browse', 'Failed to browse directory', err, { details: { dir: req.query.dir } });
      res.status(400).json({ error: `Cannot browse directory: ${err.message}` });
    }
  });

  /**
   * GET /api/database-import/scan
  ...
  router.get('/scan', async (req, res) => {
    try {
      ...
      logError(pool, 'GET /api/database-import/scan', 'Failed to scan for Access databases', err);
      res.status(500).json({ error: 'Failed to scan for databases' });
    }
  });

  /**
   * GET /api/database-import/database/:path
  ...
  router.get('/database', async (req, res) => {
    try {
      ...
      const result = await withComLock(async () => {
        ...
        // If .mdb, check if a converted .accdb already exists.
        // Don't convert now — conversion uses Access.Application which can hang
        // on .mdb files with PtrSafe errors. The listing scripts use DAO directly
        // for .mdb files, so conversion is only needed at import time.
        if (dbPath.toLowerCase().endsWith('.mdb')) {
          const accdbPath = dbPath.replace(/\.mdb$/i, '.accdb');
          try {
            await fs.access(accdbPath);
            console.log(`[COM] Using existing .accdb for ${dbName}: ${accdbPath}`);
            convertedFrom = dbPath;
            dbPath = accdbPath;
          } catch {
            // No existing .accdb — list directly from .mdb via DAO
            console.log(`[COM] Listing objects directly from .mdb for ${dbName} (no conversion)`);
          }
        }
        ...
        // Fix PtrSafe declarations before any Access.Application export operations.
        // Comments out original Declare lines and adds PtrSafe versions below them.
        let ptrSafeFixed = 0;
        try {
          console.log(`[COM] Fixing PtrSafe declarations for ${dbName}...`);
          const fixScript = path.join(scriptsDir, 'fix_ptrsafe.ps1');
          const fixOutput = await runPowerShell(fixScript, ['-DatabasePath', dbPath], { timeout: 120000 });
          const fixResult = JSON.parse(fixOutput);
          if (fixResult.success) {
            ptrSafeFixed = fixResult.declarationsFixed || 0;
            if (ptrSafeFixed > 0) {
              console.log(`[PtrSafe] Fixed ${ptrSafeFixed} declaration(s) in ${fixResult.modulesFixed} module(s)`);
            } else {
              console.log(`[PtrSafe] No fixes needed for ${dbName}`);
            }
          } else {
            console.error(`[PtrSafe] Failed: ${fixResult.error}`);
          }
        } catch (err) {
        ...
        if (ptrSafeFixed > 0) {
          response.ptrSafeFixed = ptrSafeFixed;
        }
      ...
      logError(pool, 'GET /api/database-import/database', 'Failed to get database details', err, { details: { path: req.query.path } });
