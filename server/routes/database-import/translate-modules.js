/**
 * POST /translate-modules — Server-side batch extract→map→resolve pipeline for all modules.
 * Extracts intents, maps them, and resolves gaps. No CLJS generation (JS handlers are
 * generated by vba-to-js.js at module save time).
 */
...
module.exports = function(router, pool, secrets) {
  router.post('/translate-modules', async (req, res) => {
    ...
    try {
      ...
      // 5. Log to import_log if run_id provided
      if (run_id) {
        try {
          await pool.query(`
            ...
            VALUES ($1, 6, $2, '_translate', 'system', 'issue', 'info', 'module-translation',
                    $3, 'translate')
          `, [run_id, database_id,
              `Translated ${results.extracted} modules: ${results.mapped} mapped, ${results.failed.length} failed, ${results.gaps_resolved} gaps resolved`]);
        } catch (e) {
          console.error('Error logging translation results:', e.message);
        }
      }

      console.log(`[translate-modules] Complete: ${results.extracted} extracted, ${results.mapped} mapped, ${results.failed.length} failed`);
      res.json(results);
    } catch (err) {
