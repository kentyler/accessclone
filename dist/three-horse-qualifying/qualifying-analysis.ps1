...
try {
    foreach ($currentPath in $resolvedPaths) {
        ...
        # Close DAO before opening Access.Application
        $db.Close()
        ...
        Write-Host "  Opening with Access.Application..." -ForegroundColor Gray
        $accessApp = New-Object -ComObject Access.Application
        $accessApp.AutomationSecurity = 3
        $accessApp.Visible = $false
        $accessApp.OpenCurrentDatabase($currentPath)

        # Forms
        Write-Host "  Reading forms..." -ForegroundColor Gray
        foreach ($f in $accessApp.CurrentProject.AllForms) {
            $formNames += $f.Name
            $fileStats.forms++
        }

        # Reports
        Write-Host "  Reading reports..." -ForegroundColor Gray
        foreach ($r in $accessApp.CurrentProject.AllReports) {
            $reportNames += $r.Name
            $fileStats.reports++
        }

        # Modules
        Write-Host "  Reading modules..." -ForegroundColor Gray
        ...
        # Macros
        Write-Host "  Reading macros..." -ForegroundColor Gray
        ...
        # Form detail scan
        $currentFormNames = @()
        foreach ($f in $accessApp.CurrentProject.AllForms) { $currentFormNames += $f.Name }

        if ($currentFormNames.Count -gt 0) {
            Write-Host "  Scanning form complexity..." -ForegroundColor Gray
        ...
        $currentModules = $modules | Where-Object { $_.sourceFile -eq $currentFileName }
        if ($currentModules.Count -gt 0) {
            Write-Host "  Scanning VBA modules..." -ForegroundColor Gray
            foreach ($mod in $currentModules) {
                ...
                if ($mod.lineCount -gt 0) {
                    try {
                        $accessApp.DoCmd.OpenModule($mod.name)
                        $m = $accessApp.Modules($mod.name)
                        ...
                        $accessApp.DoCmd.Close(5, $mod.name)
                    } catch {}
                }
