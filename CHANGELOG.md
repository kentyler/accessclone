...
### Added
- **Multi-pass import pipeline** — import now runs 4 automatic passes: (1) faithful import, (2) repair (case-insensitive field binding fix, record-source validation, control_column_map reconciliation), (3) validation (structural + cross-object lint, subform reference checks, combo-box SQL validation), (4) design review (LLM-based analysis against configurable design patterns).
- **Unified import log** — `shared.import_issues` table migrated into `shared.import_log` with new columns (run_id, pass_number, phase, action, severity, category, message, suggestion, resolved). All 13 INSERT sites across 6 files updated. Migration runs idempotently on server startup.
- **Import run tracking** — new `shared.import_runs` table groups import operations with start/end times, status, and summary. Endpoints: `POST /api/database-import/start-run`, `POST /api/database-import/complete-run`, `GET /api/database-import/run/:runId`.
- **Design check system** — standalone capability accessible from import (pass 4), App Viewer button, and chat tool (`run_design_check`). 12 configurable checks in `settings/design-patterns.json` across architecture, UX, and LLM-legibility categories. Endpoints: `GET/PUT /api/design-check/patterns`, `POST /api/design-check/run`.
- **Enhanced import log panel** — groups entries by pass number, color-coded severity, design recommendations display.
- **SaveAsText-based image extraction** — rewrote `export_images.ps1` to use `Application.SaveAsText` instead of COM `PictureData` property reading (which never worked). Stack-based parser extracts `PictureData` hex blocks from text output. MSysResources attachment fields accessed via child recordset + `SaveToFile`. DIB (raw BITMAPINFOHEADER) format detected and wrapped with BMP file header for browser rendering. Shared image references resolved by tracking `Picture` properties during parsing.
- **Auto-apply assessment fixes** — assessment findings are now applied automatically during import (no user decisions needed). New `POST /api/database-import/apply-fixes` endpoint. Assessment widget simplified to read-only informational display.
...
- **Relationship extraction** — new `list_relationships.ps1` PowerShell script extracts Access database relationships via DAO. Added to `GET /api/database-import/database` response.
- **Assessment endpoint** — `POST /api/database-import/assess` accepts scan data and returns grouped findings with fixability flags and suggestions.
- **LLM-enhanced assessment analysis** — after deterministic assessment, findings and scan summary are automatically sent to the LLM for domain-aware analysis (what is this database for, which issues matter most). Assessment context threaded into the chat system prompt.
- **AI agent import skill** — `skills/ai-import.md` documents the full import pipeline for AI coding agents, with two paths: full-pipeline (Claude Code, local machine access) and post-extraction (Codex, cloud sandbox).
- **Unified corpus schema** — `shared.corpus_entries` expanded with `medium` column (default 'note') to distinguish notes, messages, emails, and meetings in a single table. Added `author`, `recipients`, `thread_id`, `session_id`, `subject`, `metadata` columns. Partial indexes for zero cost on existing notes.
- **Multi-model LLM routing** — secretary model (Claude Opus) routes entries to the most appropriate responder based on content. LLM registry in `settings/config.json` defines available models (Claude Opus, Claude Sonnet, GPT-5.2, Gemini 3.1 Pro) with provider, description, and config. `server/lib/llm-router.js` handles multi-provider dispatch (Anthropic, OpenAI, Google).
- **Corpus embeddings** — `server/lib/embeddings.js` with dual-provider support (OpenAI first, Gemini fallback), 768-dim vectors via pgvector. Semantic retrieval replaces recency-based context window. Graceful degradation if pgvector unavailable.
- **Regenerate responses** — `POST /api/notes/:id/regenerate` re-generates a corpus response with user-chosen model, temperature, and sampling strategy. Response conditions UI shows editable dropdowns per response card.
