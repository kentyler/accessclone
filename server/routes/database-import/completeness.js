...
module.exports = function(router, pool) {
  ...
  /**
   * PUT /api/database-import/source-discovery
   */
  router.put('/source-discovery', async (req, res) => {
    try {
      ...
      logError(pool, 'PUT /api/database-import/source-discovery', 'Failed to save source discovery', err);
      res.status(500).json({ error: 'Failed to save source discovery' });
    }
  });

  /**
   * GET /api/database-import/import-completeness
   */
  router.get('/import-completeness', async (req, res) => {
    try {
      ...
      logError(pool, 'GET /api/database-import/import-completeness', 'Failed to check import completeness', err);
  ...
  router.get('/history', async (req, res) => {
    try {
      ...
      logError(pool, 'GET /api/database-import/history', 'Failed to fetch import history', err);
