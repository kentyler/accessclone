...
- **Import run tracking** — new `shared.import_runs` table groups import operations with start/end times, status, and summary. Endpoints: `POST /api/access-import/start-run`, `POST /api/access-import/complete-run`, `GET /api/access-import/run/:runId`.
...
- **Auto-apply assessment fixes** — assessment findings are now applied automatically during import (no user decisions needed). New `POST /api/access-import/apply-fixes` endpoint. Assessment widget simplified to read-only informational display.
