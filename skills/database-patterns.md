...
The ClojureScript/Reagent frontend calls PG functions through the session-state pattern:
1. Creates a session via `POST /api/session`
2. Sets input state via `PUT /api/form-state`
3. Calls the function via `POST /api/queries/execute`
4. Checks for `user_message` → display if present
5. Reads results from session state
6. Clears session when done

> **Note**: VBA event handlers (button clicks, form events) now execute as JavaScript via `window.AC` runtime — see `skills/event-runtime.md`. The session-state PG function pattern described here is for server-side business logic that has been explicitly translated to PostgreSQL functions.
...
1. Collect changed fields from the current record
2. POST/PUT to `/api/data/:table` with the record data
3. Handle errors via the `log-error!` pattern

### Button Actions → Event Handlers

Form button clicks are handled by JavaScript generated from VBA at import time:

```javascript
// Generated by vba-to-js.js from: Sub btnGenerateCandidates_Click()
AC.openForm("frmCandidates");
AC.setValue("recipe_id", AC.getValue("current_recipe"));
```

The `window.AC` runtime API bridges generated JS to the ClojureScript app state. See `skills/event-runtime.md` for the full API.
