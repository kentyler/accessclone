...
**VBA Intent Extraction Pipeline** (`server/lib/vba-intent-*.js`):
Intent extraction for LLM reasoning: (1) `vba-intent-extractor.js` sends VBA to Claude Sonnet, gets structured JSON intents; (2) `vba-intent-mapper.js` maps 30 intent types deterministically. Intents are used for LLM reasoning context and gap decisions, not runtime execution. Runtime event handlers are JavaScript generated by `vba-to-js.js`.
...
- **Modules**: VBA source + JS handlers + intent structure + app object inventory. Also: intent extraction endpoint (`/api/chat/extract-intents`) for structured intent extraction
